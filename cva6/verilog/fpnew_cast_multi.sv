...
module fpnew_cast_multi #(
  ...
  for (genvar i = 0; i < NUM_INP_REGS; i++) begin : gen_input_pipeline
    ...
    // (computed in the `inp_pipe_ready_chain` process below: element i reads
    // element i+1, which a LATER iteration produced, so as concurrent
    // assigns there is no order a linearizing front end can follow.)
  ...
  // The whole inp_pipe_ready chain as ONE process: a process orders its own
  // statements, so the descending scan writes each element before the
  // one that reads it -- the same fixpoint, in a followable order.
  always_comb begin : inp_pipe_ready_chain
    inp_pipe_ready[NUM_INP_REGS] = mid_pipe_ready[0];
    for (int unsigned i = NUM_INP_REGS; i > 0; i--) begin
      inp_pipe_ready[i-1] = inp_pipe_ready[i] | ~inp_pipe_valid_q[i];
    end
  end
  ...
  for (genvar i = 0; i < NUM_MID_REGS; i++) begin : gen_inside_pipeline
    ...
    // 1. if the next stage is ready for our data
    // 2. if the next stage only holds a bubble (not valid) -> we can pop it
    // (computed in the `mid_pipe_ready_chain` process below: element i reads
    // element i+1, which a LATER iteration produced, so as concurrent
    // assigns there is no order a linearizing front end can follow.)
    // Valid: enabled by ready signal, synchronous clear with the flush signal
  ...
  // The whole mid_pipe_ready chain as ONE process: a process orders its own
  // statements, so the descending scan writes each element before the
  // one that reads it -- the same fixpoint, in a followable order.
  always_comb begin : mid_pipe_ready_chain
    mid_pipe_ready[NUM_MID_REGS] = out_pipe_ready[0];
    for (int unsigned i = NUM_MID_REGS; i > 0; i--) begin
      mid_pipe_ready[i-1] = mid_pipe_ready[i] | ~mid_pipe_valid_q[i];
    end
  end
  // Generate the register stages
  for (genvar i = 0; i < NUM_OUT_REGS; i++) begin : gen_output_pipeline
    ...
    // 1. if the next stage is ready for our data
    // 2. if the next stage only holds a bubble (not valid) -> we can pop it
    // (computed in the `out_pipe_ready_chain` process below: element i reads
    // element i+1, which a LATER iteration produced, so as concurrent
    // assigns there is no order a linearizing front end can follow.)
    // Valid: enabled by ready signal, synchronous clear with the flush signal
  ...
  // The whole out_pipe_ready chain as ONE process: a process orders its own
  // statements, so the descending scan writes each element before the
  // one that reads it -- the same fixpoint, in a followable order.
  always_comb begin : out_pipe_ready_chain
    out_pipe_ready[NUM_OUT_REGS] = out_ready_i;
    for (int unsigned i = NUM_OUT_REGS; i > 0; i--) begin
      out_pipe_ready[i-1] = out_pipe_ready[i] | ~out_pipe_valid_q[i];
    end
  end
