...
FP8. Please use the PULP DivSqrt unit when in need of div/sqrt operations on FP8.");
  ...
  if (OpGroup == fpnew_pkg::CONV) begin : target_regs
    ...
    for (genvar i = 0; i < NumPipeRegs; i++) begin : gen_bypass_pipeline
      ...
      // (computed in the `byp_pipe_ready_chain` process below: element i reads
    ...
    // The whole byp_pipe_ready chain as ONE process: a process orders its own
    ...
    always_comb begin : byp_pipe_ready_chain
      byp_pipe_ready[NumPipeRegs] = out_ready_i & result_is_vector;
      for (int unsigned i = NumPipeRegs; i > 0; i--) begin
        byp_pipe_ready[i-1] = byp_pipe_ready[i] | ~byp_pipe_valid_q[i];
      end
    end
