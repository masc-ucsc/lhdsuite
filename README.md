...
| `synth_lec_flat` / `synth_lec_synth` | netlist integrity: synthesize twice (2nd run after a comment touch — under `color synth` that netlist is largely cache-CLONED), then `lhd lec --impl lg:netlist --ref lg:design --lib lg:models` proves it against the compiled design (`pass liberty gensim` provides the sky130 cell models; strict, so UNKNOWN fails). For `lec_synth` the design side is the one-shot's own `W/synth/lg`. Only generated for colorings the core actually runs, so minion has no `synth_lec_flat` |
| `sim_verilog` / `sim_pyrope` | asserted `lhd sim` benchmark: dino runs `dino_prog_tb` on the whole CPU for 1M cycles; minion runs `minion_prog_tb` on the whole core for 100k cycles. The Verilog side compiles straight to an `lg:` library rooted at the module each driver drives. Host C++ compilation and simulation are timed separately, and the timed path disables VCD. cva6 runs `tag_cmp_tb` on the tag_cmp cone for 200 cycles. Additional whole-top correctness drivers are reported separately; dino gates on them, while minion's and cva6's remain informational — minion's is blocked by a derived clock that sim cgen cannot fold, cva6's (`cva6_prog_tb`, a real RISC-V program on the FULL core over AXI, 50k cycles) by the generated icache. cva6's program driver is `pyrope`-only: its DUT is the whole `cva6` module, which lives in `cva6/pyrope/` but not in `cva6/verilog/`, so `sim_verilog` skips it |
...
rows, `cva6_sim_incremental`, the `sim_cpu_prog_ok` metric of
`cva6_sim_pyrope`, and — added 2026-08-21 — `genprp_minion` plus the two
xiangshan genprp rows (`genprp_xs_enqentry_4`, `genprp_xs_tracebuffer`).
Two rows have LEFT this table: `dino_sim_verilog`, whose
`io_in` mismatch turned out to be suite-side and is fixed (`sim_tb_v`, below),
and **`cva6_lec_bug`, whose FALSE PROVEN is gone** — the injected bug is now
REFUTED with a counterexample (`hit_way_o(ref=0 impl=1)`, 294 ms). That is a
livehd-side fix, not a suite one: re-measured 2026-08-21, it refutes at the OLD
3-port configuration too (HEAD's wrapper, variant and Verilog), so nothing about
the cva6 rework below is what made it pass:

| target | blocked by |
| --- | --- |
| `cva6_sim_pyrope` METRIC `sim_cpu_prog_ok` | **the generated `cva6/pyrope/cva6_icache.prp` mistranslates every fetch address, so the core never retires an instruction.** `cva6/sim/cva6_prog_tb.prp` drives the whole core over AXI with a real RISC-V program (assembled with `riscv64-unknown-elf-as`, published back on an uncached window at 0x1000_0000). The core resets, `i_frontend.npc_q` reaches `boot_addr_i` = 0x8000_0000, the icache completes its 256-entry flush and issues an AXI read — with `ar_addr` = 0. Two root causes are confirmed by simulation, a third is not: (1) **a stale `mut` read.** `dreq_o.ready` is emitted as a `mut` seeded 0 at the top of the body, READ at the `vaddr_d`/`cl_offset_d` muxes (`cva6_icache.prp:437,441`) and only ASSIGNED later inside the FSM (`:468,490`). Pyrope `mut` is last-write-in-PROGRAM-ORDER, so those reads always see 0, `vaddr_q` never latches `dreq_i.vaddr`, and the icache hands the MMU address 0 to translate. In the SystemVerilog these are continuous assigns seeing the settled value — the emitted net needed to be a `wire` (readable above its driver), exactly as `mem_data_req_o` in the same file correctly is. Reproduce in seconds without the core: sim `cva6_icache` standalone driving `dreq_i.req=1, dreq_i.vaddr=0x8000_0000` and watch `vaddr_q` stay 0 while `dreq_o.ready` pulses 1. (2) **an unbound inlined argument.** `address`, the argument of the inlined `is_inside_cacheable_regions` check, is assigned `0sb?` (`:398`) and never bound to `{cl_tag_d, 12'b0}`, so the cacheable-region test is unknown and every fetch takes the non-cacheable path. (3) With both patched in a scratch tree the address and burst shape become correct (0x8000_0000, `ar_len`=1, a 128-bit line), the driver returns both beats, and the icache still re-requests forever with `icache_valid_q` stuck at 0 — at least one more defect in the line-fill path, not yet root-caused. Two more observations, neither of them the blocker: `noc_req_o.ar_size`/`ar_burst` read 0 at the cva6 top while `cva6_icache_axi_wrapper` drives 3 and a valid burst at its own `axi_req_o` (bits 80..84 of the packed AR are lost across `std_cache_subsystem`'s stream_arbiter; bits 85+ survive, which is why `ar_len` and `ar_addr` do), and `--set sim.vcd=true` on `cva6.cva6` is refused outright — *"module `cva6.cva6` cannot be lowered by the occurrence-wide color scheduler / a literal-only output contains runtime-unknown bits"* — independent of the driver, so under the bench (which traces these untimed drivers) the failure surfaces there first. `cva6` leaves `sim_top_assert` unset, so all of this is a metric rather than a red target. Do NOT weaken the driver's asserts: `x2 == 100` / `x3 == -100` is plain RISC-V semantics |
| `cva6_sim_incremental` | **cva6's `bug1` is detectable by its own sim driver, and `sim.sh`'s MODE=incr pass 3 does not allow for that.** Pass 3 applies `tests/bug1` (way 0 hits regardless of the line's valid bit) and re-runs the driver. `tag_cmp_tb.prp` drives an all-invalid array and asserts `hit_way_o == 0`, so under bug1 it correctly fails — `got=1 == 0` — `lhd sim` exits nonzero, and `run_timed … \|\| return 1` kills the pass. This is PRE-EXISTING and suite-side, not a livehd gap: verified 2026-08-21 by running HEAD's wrapper + HEAD's bug1 variant + HEAD's driver, which fails identically. dino does not hit it because its `bug1` (the ALU's 32-bit add flipped to subtract) does not disturb `dino_prog_tb`'s assertions, so `dino_sim_incremental` passes. The fix belongs in `bench/sim.sh` and mirrors what pass 3 already does for the checksum: `sim_pass edit 0` passes `SIM_GATE_EXPECT=0` precisely because *"bug1 changes the design, so the checksum MUST move… keeping the pass-1 checksum as a gate there would fail the target for doing its job."* The driver's EXIT STATUS needs the same relaxation on that pass — tolerate a nonzero exit as long as the driver still printed `$CORE_SIM_MARKER` (proving it ran rather than crashed), while `generated_newer` keeps doing the real job of proving something was rebuilt |
...
| --- | --- | --- | --- | --- |
| `top` — whole-design top, in both languages | `PipelinedDualIssueCPU` | `minion_top` | `tag_cmp_wrap` | `matched_filter` |
| `unit` — module carrying the verify sidecar + `bug1`/`comment1` (must be in the top's cone) | `ALU` | `txfma_adder` | `tag_cmp_wrap` | `tap` |
...
| `sim_tb` / `sim_cycles` — asserted, timed sim benchmark | `dino_prog_tb.prp` / 1,000,000 | `minion_prog_tb.prp` / 100,000 | `tag_cmp_tb.prp` / 200 | `matched_filter_tb.prp` / 5,000,000 |
| `sim_tb_unit` — the module `sim_tb` drives, i.e. what it spells in `import("lg:NAME")`; the design supplied before it is rooted HERE, not at `top` | `PipelinedDualIssueCPU` | `minion_top` | `tag_cmp_wrap` | `matched_filter` |
