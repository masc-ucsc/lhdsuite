...
design: `cva6/verilog/` is the cache tag-comparator cone, carrying CVA6's own
SVA, and every scenario needing both sides runs on that; `cva6/pyrope/` is the
WHOLE core, generated by the recipe in `cva6/README`, and only the pyrope-only
`cva6_prog_tb.prp` sim driver reaches it. See `cva6/BUILD`'s SCOPE comment.
