...
int main(int argc, char** argv) {
  ...
  // Same boot sequence and always-active loop as minion_prog_tb.prp: line N
  // repeats `addi x(N+1),x0,0` for N=0..30, line 31 is a nop barrier, and
  // every later line repeats the active loop.
  const uint32_t loop_rom[8] = {
      0x00110113u,
      0x00318193u,
      0x00314233u,
      0x002202B3u,
  ...
  int      resp_latency = 5;
  if (const char* e = std::getenv("MINION_TB_LAT")) {
    resp_latency = std::atoi(e);
  }
  constexpr int kMaxLat = 16;
  int           resp_line_at[kMaxLat];  // -1 = no response scheduled for that slot
  for (int i = 0; i < kMaxLat; ++i) {
    resp_line_at[i] = -1;
  }
  int fetch_line = 0;
  ...
  for (long clock = 0; clock < cycles; ++clock) {
    ...
    for (int i = 0; i < 8; ++i) {  // data = bits [260:5]
      uint32_t instr = 0x00000013u;
      if (fetch_line < 31) {
        instr = (static_cast<uint32_t>(fetch_line + 1) << 7) | 0x13u;
      } else if (fetch_line > 31) {
        instr = loop_rom[i];
      }
      set_bits(dut->icache_resp_i.data(), 5 + 32 * i, 32, instr);
    }
    ...
    if (dut->icache_req_valid_o != 0) {
      ...
      resp_line_at[(clock + resp_latency) % kMaxLat] = static_cast<int>(addr >> 5);
    }
    if (clock < 60 && std::getenv("MINION_TB_DEBUG") != nullptr) {
      std::printf("c%ld req_v=%d addr=0x%llx resp_v=%d retired=%ld",
                  clock,
                  static_cast<int>(dut->icache_req_valid_o),
                  static_cast<unsigned long long>(dut->icache_req_o & ((1ULL << 49) - 1)),
                  static_cast<int>(dut->icache_resp_valid_i),
                  retired);
#ifdef MINION_TB_INTERNALS
      auto* r = dut->rootp;
      std::printf(
          " | f6rq=%d f6rv=%d corereq=%d halt0=%d bwr=%d",
          static_cast<int>(
              r->minion_top__DOT__u_core__DOT__u_frontend__DOT__gen_thread_buf__BRA__0__KET____DOT__u_tb__DOT__f6_req_valid),
          ...
          static_cast<int>(
              r->minion_top__DOT__u_core__DOT__u_frontend__DOT__gen_thread_buf__BRA__0__KET____DOT__u_tb__DOT__io_halt_i),
          static_cast<int>(
              r->minion_top__DOT__u_core__DOT__u_frontend__DOT__gen_thread_buf__BRA__0__KET____DOT__u_tb__DOT__f6_buffer_wr));
#endif
      std::printf("\n");
    }
    ...
    if (get_bits(te, 60, 1) != 0 || get_bits(te, 54, 1) != 0) {  // exception | interrupt
      ...
      if (std::getenv("MINION_TB_DEBUG") != nullptr) {
        std::printf("  exc @c%ld: exception=%d interrupt=%d cause=0x%llx trap_pc=0x%llx tval=0x%llx\n",
                    clock,
                    static_cast<int>(get_bits(te, 60, 1)),
                    static_cast<int>(get_bits(te, 54, 1)),
  ...
  const bool ok = retired >= 100 && last_pc >= 1024 && last_pc < 1088;
  if (std::getenv("MINION_TB_DEBUG") != nullptr) {
